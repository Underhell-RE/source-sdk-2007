...
## Free-aim — implemented (stage 1: client)

First-person free-aim: the mouse moves a free-aim cursor (decoupled from the
view), the crosshair draws at the cursor, and the weapon viewmodel tilts a few
degrees toward it. Decoded from `CBaseViewModel::CalcViewModelView` =
`sub_10014D80` (the tilt math) + the VDC tutorial the mod author shipped in
`notes/` (the input/cursor/convars).

### What was ported (all client-side, gated by cam_ots_freeaim_enable)

- `iinput.h` / `input.h`: `CAM_IsFreeAiming()` + `CAM_GetFreeAimCursor()` on the
  input interface, `Vector2D m_vecFreeAimPos` cursor member.
- `in_main.cpp`: the 7 `cam_ots_freeaim_*` ConVars (defaults from Cliento.dll).
- `in_mouse.cpp`: `CInput::TryCursorMove()` — accumulate the cursor from mouse
  deltas (m_yaw/m_pitch/FOV-scaled), deadzone (`move_threshold`) + auto-turn
  past `move_max`, clamp cursor to `move_max` (0.1). `MouseMove` routes to it
  when `CAM_IsFreeAiming()`, else `ApplyMouse`.
- `in_camera.cpp`: `CAM_IsFreeAiming` = convar && local player;
  `CAM_GetFreeAimCursor` returns the cursor.
- `hud_crosshair.cpp`: crosshair drawn at the cursor (`x *= cursor.x + 1`).
- `baseviewmodel_shared.cpp` `CalcViewModelView`: after the ironsight slide, if
  free-aim enabled && !m_bExpSighted, re-aim the viewmodel at
  `ScreenToWorld((cursor*0.25+0.5)*screensize)` — the ~1–2° weapon tilt from
  sub_10014D80 (with the far-off-axis 10% blend branch).

### Divergences / TODO (documented)

- The cursor magnitude is clamped to `cam_ots_freeaim_move_max` (0.1), so the
  tilt is subtle (a few degrees) — matches the "slightly tilts" behaviour.
- `cam_ots_TurnAuto` / `cam_ots_Turn180` commands + the smooth
  `CAM_UpdateAngleByFreeAiming`/`CAM_UpdateAngle180` (used for the interval
  autoturn) are NOT ported — the non-interval autoturn in TryCursorMove covers
  the default path.
- The server side (`update_freeaim` ConCommand + aim tracking, needed for the
  networked aim direction / prediction parity) is stage 2 — the viewmodel tilt
  and crosshair are purely client-side, so stage 1 is self-contained visually.

### Test in game

- `cam_ots_freeaim_enable 0/1` — toggle the whole feature.
- Move the mouse without aiming: the weapon should tilt a couple of degrees
  toward the mouse and settle back; the crosshair drifts slightly toward the
  cursor.
- With a two-handed weapon, ironsighting should suppress the tilt (m_bExpSighted).
