...
- Slot structure (ctor, sub_1012E360): 28 `DragnDropSlot` (ImageButton/ImagePanel) children, each 28x28. 4x6 grid is **column-major** (`index = row + col*6`), origin **(44, 119)**, pitch 37. Extra slots 24..27 at (82,118), (82,81), (343,118), (343,81). Every length goes through `ISchemeManager::GetProportionalScaledValue` (640x480 base). Frame is proportional so the 1024x512 `.res` / `Inventory.vtf` scales with the slots. Sprites are scheme images at `../Sprites/Hud/...` (vgui-rooted). Tooltips carry the `#UnderHell_Inventory_*` tokens — no per-slot name label.
