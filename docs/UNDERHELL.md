...
- Slot structure: `Inventory.vtf` is 1024x512; the opaque rounded rect is an **8×4 pocket grid** (pitch ~84, origin ~178,91 — measured from the DXT5, not the garbled 28/37/44/119 hexrays literals). 28 game slots fill **row-major** (3 full rows + 4 in the last). Frame is not proportional. LMB/RMB Use/Drop menu as before.
