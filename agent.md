...
5. **Фонарик** — включается только со свободной левой рукой (одноручное/холодное);
   с двухручным — отказ. Починено: `UH_UpdateLeftArm` теперь использует тот же
   гейт `!pWeapon || OneHanded || MeleeWeapon`, что и `FlashlightTurnOn`
   (раньше без оружия свет горел, а вьюмодель в левой руке не поднималась).
